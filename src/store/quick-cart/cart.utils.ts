...
export type UpdateItemInput = Partial<Omit<Item, '_id'>>
...
export const calculatePa_idTotal = (
  { totalAmount, tax, shipping_charge }: PriceValues,
  discount?: number
) => {
  let pa_idTotal = totalAmount + tax + shipping_charge;
  if (discount) {
    pa_idTotal = pa_idTotal - discount;
  }
  return pa_idTotal;
};
