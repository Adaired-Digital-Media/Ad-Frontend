...
const NavItems = () => {
  ...
  const navItems = useMemo(() => {
    return pathname.startsWith('/ecommerce') ? routes.ecommerceNav : routes.websiteNav;
  }, [pathname]);
...
const Item = ({ activeIndex, navitems }: { navitems: HeaderItems; activeIndex: () => void }) => {
      ...
      return (
        <div
          className={cn(
            ...
            submenuClicked ? '' : 'group-hover:pointer-events-auto group-hover:scale-y-100'
          )}
        >
                          ...
                          <li key={subSubItem.name}>
                            ...
                            >
                              <div className="flex items-center gap-2 text-gray-400 transition-all duration-300 group-hover/subMenu:text-[#FB9100]">
                                ...
                                <span className="flex text-base">{subSubItem.name}</span>
                              </div>
            ...
            {navitems.childrens.map((children) => (
              <div key={children.name} className="group/children">
                <Link
                  ...
                  onClick={handleSubmenuClick}
                >
                  <span className={cn('flex items-center font-nunito text-base group-hover/children:text-[#FB9100]')}>
                    {children.name}
